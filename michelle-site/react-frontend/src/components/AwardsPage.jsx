...
export default function SocialWork() {
  ...
  useEffect(() => {
    axios.get('http://127.0.0.1:8000/api/awards-page')
            ...
            {items.map(item => (
              <div key={item.id} className="edu-gallery-item">
                {item.images.length > 0 && (
                  <img
                    src={`http://127.0.0.1:8000/storage/${item.images[0].url}`}
                    alt=""
      ...
      {modalItem && (
        <div className="edu-modal" onClick={closeModal}>
          <div className="edu-modal-content" onClick={e => e.stopPropagation()}>
            ...
            <img
              src={`http://127.0.0.1:8000/storage/${modalItem.images[currentSlide].url}`}
              alt={modalItem.title}
